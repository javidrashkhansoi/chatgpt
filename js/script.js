  ...
  #highlight(block, link) {
    ...
    const blockResizeObserver = new ResizeObserver(entries => {
      entries.forEach(entry => {
        ...
        function threshold() {
          let threshold = (document.documentElement.clientHeight / 2) / target.clientHeight
...
;// CONCATENATED MODULE: ./src/js/libraries/swiper/sliders/modules.js


const modulesSlider = new core(".modules-slider", {
  allowTouchMove: false,
  autoHeight: true,
  spaceBetween: 30,
});

const modulesButtons = document.querySelectorAll(".modules__button");

modulesButtons?.forEach((currentButton, index, buttons) => {
  currentButton.addEventListener("click", () => {
    const { slide } = currentButton.dataset;

    buttons.forEach(button => {
      button.classList.toggle("modules__button--active", currentButton === button);
    });

    if (slide) {
      modulesSlider.slideTo(+slide - 1, 500, false);
    }
  });
});
